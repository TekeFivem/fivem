...
local function vmoney(n) return math.floor((n or 0) + 0.5) end
local function vacc() return VC.account or 'bank' end
local function truthy(v) return v == 1 or v == true or v == '1' end
  ...
  for _, pid in ipairs(GetPlayers()) do
    ...
    if sid and sid ~= src then
      ...
      if tped and tped ~= 0 then
        ...
        if d <= radius and (not bestDist or d < bestDist) then best, bestDist = sid, d end
      end
    end
  end
  return best
...
local function uiSecurity(s) return (s == 'unprotected') and 'none' or s end

local function labelOf(name)
  local ok, def = pcall(function() return exports.ox_inventory:Items(name) end)
  if ok and def and def.label then return def.label end
  return name
end
...
local function genLevels(tier)
  ...
  return {
    value = rollLevel(bias), rarity = rollLevel(bias),
    clean = rollLevel(neu), repair = rollLevel(neu),
    authentic = rollLevel(neu), demand = rollLevel(neu), legal = rollLevel(neu),
  }
end
...
lib.callback.register('teke_auction:getVault', function(source)
  ...
  local rows = MySQL.query.await([[
    ...
    FROM vault_boxes WHERE owner_id = ? ORDER BY opened ASC, end_time ASC LIMIT 20
  ]], { cid })
  local out = {}
  for _, r in ipairs(rows or {}) do
    local cleaned = truthy(r.opened)
    local remaining = math.max(0, r.end_time - os.time())
    out[#out+1] = {
      id = tostring(r.id), kind = r.kind, tier = r.tier,
      name = r.name or ((PREFIX[r.kind] or 'BOX') .. '-' .. r.id),
      estValue = r.est_value, bid = r.est_value,
      security = uiSecurity(r.security),
      endTime = cleaned and '00:00:00' or os.date('!%H:%M:%S', remaining),
      participants = 0, cleaned = cleaned,
...
lib.callback.register('teke_auction:cleanBox', function(source, data)
  ...
  if method == 'cleaner' then
    ...
    if not c then return { ok = false, reason = 'badtier' } end
    cost, theft = c.price or 0, c.theft or 0
    if (player.PlayerData.money[vacc()] or 0) < cost then return { ok = false, reason = 'money', cost = cost } end
  else
  ...
  local upd = MySQL.update.await(
    ...
    { boxId, cid })
  if not upd or upd < 1 then return { ok = false, reason = 'notfound' } end

  if cost > 0 then player.Functions.RemoveMoney(vacc(), cost, 'vault-clean') end
  ...
  TriggerClientEvent('teke_auction:vaultBoxOpened', src, { id = tostring(boxId) })
  return { ok = true, boxId = tostring(boxId), made = made }
end)
...
lib.callback.register('teke_auction:getBoxLoot', function(source, data)
  ...
  local player = exports.qbx_core:GetPlayer(source)
  if not player then return { items = {}, identifySeconds = secs } end
  local cid = player.PlayerData.citizenid
  local boxId = tonumber(data and data.id)
  if not boxId then return { items = {}, identifySeconds = secs } end
  local rows = MySQL.query.await(
  ...
  for _, r in ipairs(rows or {}) do
    if truthy(r.identified) then
      ...
      items[#items+1] = { id = tostring(r.id), identified = false }
    end
  end
  return { items = items, identifySeconds = secs }
end)
...
lib.callback.register('teke_auction:getLoot', function(source)
  ...
  local out = {}
  for _, r in ipairs(rows or {}) do out[#out+1] = lootObj(r) end
  return out
end)
...
lib.callback.register('teke_auction:vaultInsure', function(source, data)
  ...
  if not row then return { ok = false, reason = 'notfound' } end
  if row.security ~= 'unprotected' then return { ok = false, reason = 'already' } end
  ...
  return { ok = true, security = 'insured', paid = cost }
end)
...
lib.callback.register('teke_auction:vaultSecure', function(source, data)
  ...
  local row = ownedOpenBox(cid, boxId)
  if not row then return { ok = false, reason = 'notfound' } end
  if row.security == 'secured' then return { ok = false, reason = 'already' } end
  if row.security ~= 'insured' then return { ok = false, reason = 'needinsurance' } end
  ...
  return { ok = true, security = 'secured', paid = cost }
end)
...
lib.callback.register('teke_auction:vaultExtend', function(source, data)
  ...
  return { ok = true, endTime = newEnd, paid = cost }
end)
...
lib.callback.register('teke_auction:vaultSellSystem', function(source, data)
  ...
  if not del or del < 1 then return { ok = false, reason = 'notfound' } end
  player.Functions.AddMoney(vacc(), offer, 'vault-sell-system')
  return { ok = true, paid = offer }
end)
...
lib.callback.register('teke_auction:vaultSellPlayer', function(source, data)
  ...
  if not buyer or not exports.qbx_core:GetPlayer(buyer) then return { ok = false, reason = 'nobuyer' } end
  pendingOffers[buyer] = { boxId = boxId, price = price, sellerCid = cid, sellerSrc = src, expires = os.time() + 30 }
  TriggerClientEvent('teke_auction:vaultOffer', buyer, {
    boxId = tostring(boxId), name = row.name, price = price, seller = Payouts.nameOf(cid),
  })
...
RegisterNetEvent('teke_auction:vaultOfferResponse', function(accepted)
  local buyer = source
  local offer = pendingOffers[buyer]; pendingOffers[buyer] = nil
  if not offer or os.time() > offer.expires or not accepted then return end
  local bp = exports.qbx_core:GetPlayer(buyer)
  local sp = exports.qbx_core:GetPlayer(offer.sellerSrc)
  if not bp or not sp then return end
  ...
  if not playersNear(offer.sellerSrc, buyer, VC.transferRadius or 3.0) then
    TriggerClientEvent('teke_auction:vaultToast', offer.sellerSrc, { type = 'error', msg = 'Alıcı uzaklaştı, devir iptal.' })
    return
  end
  if offer.price > 0 then
    if (bp.PlayerData.money[vacc()] or 0) < offer.price then
      TriggerClientEvent('teke_auction:vaultToast', offer.sellerSrc, { type = 'error', msg = 'Alıcının parası yetmedi.' })
      return
    end
