...
lib.callback.register('teke_auction:joinAuction', function(source, data)
  ...
  if not (a and a[1]) then return false end
  if a[1].status ~= 'open' and a[1].status ~= 'final' then return false end
  ...
  NotifyViewers('stats', { id = tostring(id), participants = count })
...
lib.callback.register('teke_auction:placeBid', function(source, data)
  ...
  if not id or amount <= 0 then return { ok = false } end

  local a = MySQL.query.await("SELECT tier, current_price, base_bid, status FROM auctions WHERE id = ?", { id })
  if not (a and a[1]) then return { ok = false } end
  local row = a[1]
  if row.status ~= 'open' and row.status ~= 'final' then return { ok = false, reason = 'phase' } end
...
lib.callback.register('teke_auction:getBids', function(_, data)
  ...
  for _, r in ipairs(rows or {}) do
    out[#out+1] = { id = tostring(r.id), player = r.bidder_name, amount = r.amount, hidden = (r.hidden == 1) }
  end
  return out
end)