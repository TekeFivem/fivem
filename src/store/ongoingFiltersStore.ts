...
interface OngoingFiltersState {
  tiers: Tier[] // boş = tümü
  kinds: AuctionKind[] // boş = tümü

  // eşik filtreleri (preset null = Hepsi), yön default 'gte' (artan)
  bidPreset: number | null
  bidDir: Dir
  timePreset: number | null // dakika
  timeDir: Dir
...
export const useOngoingFiltersStore = create<OngoingFiltersState>((set) => ({
  tiers: [],
  kinds: [],
  ...
  nextPage: (totalPages) =>
    set((s) => ({ page: Math.min(s.page + 1, totalPages - 1) })),
